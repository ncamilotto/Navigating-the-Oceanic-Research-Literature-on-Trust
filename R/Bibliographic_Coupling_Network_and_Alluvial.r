...
database_WOS <- db567 #db567 is the database created from the query on WoS, unfortunately I can't make it public but I'll be glad to send it to you. 
...
nodes <- database_WOS %>% 
				...
				subset(PY <2006) %>% #Selects articles published between 1991 and 2005
				subset(PY > 1990) #This is the parameter to be modified to obtain the Bilbiographic coupling from 2005-2020
...
nodes <- nodes %>%
				...
				subset(select = c('Noeud','Label','AU','PY','TI','SO','Z9','SC','WC')) %>% 
				rename(ID = "Noeud") %>% 
...
time_window = 15, #This is the parameter to be modified to obtain the alluvial with a time window of 5 years
edges_threshold = 2,
...
alluv_dt <- networks_to_alluv(list_graph = temporal_networks_cluster_intertemporal,
